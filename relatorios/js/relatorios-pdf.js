...
async function gerarRelatorio(modo = "final") {
  ...
  const headerVistoria = () => {
    ...
    doc.text("TIPO: Relatório de Vistoria", 1.3, 3.2);

    doc.text(`DATA: ${dFinal}`, 10.5, 3.2, {
      align: "center",
    });

    doc.text("PREPARADO POR: Albetan Reis", 14.8, 3.2);
  };
  ...
  function rodapeVistoria() {
    ...
    if (rodapeEndereco) {
      ...
      doc.text(linhasEndereco, 10.5, 28.5, {
        align: "center",
      });
    }
  }
  ...
  const modeloRelatorio = parametros.get("modelo");

  // MODELOS PERMITIDOS
  const modelosPermitidos = ["fornecedor", "vistoria"];

  if (!modelosPermitidos.includes(modeloRelatorio)) {
    throw new Error("Modelo de relatório inválido ou não informado.");
  }

  const isVistoria = modeloRelatorio === "vistoria";
  ...
  if (isVistoria) {
    ...
    const ufLoja = dadosLojaCapa["UF"] || dadosLojaCapa.uf || "";

    const cepLoja = dadosLojaCapa["CEP"] || dadosLojaCapa.cep || "";
    ...
    if (logos.prestador) {
      ...
      const xLogoCapa = (21 - larguraLogoCapa) / 2;
    ...
    doc.text("RELATÓRIO DE VISTORIA:", 10.5, 11.0, {
      align: "center",
    });

    doc.text(`LOJA ${lNome}`, 10.5, 12.1, {
      align: "center",
    });
    ...
    if (fotosObrigatorias.fachada) {
      ...
      const xFachada = (21 - larguraFachada) / 2;

      // Quadro da Fachada + legenda
      doc.setDrawColor(0);
      doc.setLineWidth(0.017);

      doc.rect(xFachada, 13.2, larguraFachada, alturaFachada + 1.1);
    ...
    if (rodapeEndereco) {
      const enderecoFormatado = doc.splitTextToSize(
        rodapeEndereco.toUpperCase(),
        18.5,
      );

      doc.text(enderecoFormatado, 10.5, 28.1, {
        align: "center",
      });
    }
  } else {
  ...
  if (isVistoria) {
    ...
    const textoObjetivo = "XXXXXXXXXXXXXXXXXXXXXXXXXXXX";

    const linhasObjetivo = doc.splitTextToSize(textoObjetivo, 18);

    doc.text(linhasObjetivo, 1.5, 7.0, {
      align: "left",
    ...
    if (fotosObrigatorias.fachada || fotosObrigatorias.marquise) {
      ...
      if (fotosObrigatorias.marquise) {
        ...
        doc.setFont("helvetica", "normal");
        doc.text(
          "Figura 2 – Marquise, Letreiro",
          1.7,
          curY + alturaImagem + 0.7,
        );
      }
    }
  }
  ...
  if (isVistoria) {
    ...
    if (
      ...
      doc.text("ÍNDICE DE ANEXOS", 10.5, 5.2, {
        align: "center",
      });
      ...
      anexos.forEach((a, i) => {
        ...
        doc.text((i + 4).toString(), 19.5, idxY, {
          align: "right",
        });
  ...
  for (let indiceAnexo = 0; indiceAnexo < anexos.length; indiceAnexo++) {
    ...
    const descricaoGeralAnexo = `Anexo ${indiceAnexo + 1} - ${anexo.obs || " "}`;
    ...
    if (isVistoria) {
      ...
      const posicoesY = [yLinha1, yLinha2, yLinha3];
      ...
      for (
        ...
        const fotosPagina = todasFotos.slice(inicio, inicio + fotosPorPagina);
        ...
        if (inicio === 0) {
          const paginaInicialAnexo = doc.getCurrentPageInfo().pageNumber;

          const numeroAnexo = indiceVistoria.length + 1;

          indiceVistoria.push({
            numero: `2.${numeroAnexo - 1}`,

            anexo: numeroAnexo,

            nome: String(anexo.obs || "").trim(),

            pagina: paginaInicialAnexo,
          });
        }
        ...
        if (descricaoGeralAnexo && descricaoGeralAnexo.trim()) {
          doc.setFont("helvetica", "normal");

          doc.setFontSize(8);

          const descricaoAnexoFormatada = doc.splitTextToSize(
            descricaoGeralAnexo,
            18,
          );

          doc.text(descricaoAnexoFormatada, 1.5, 5.35);
        }
        ...
        fotosPagina.forEach((foto, index) => {
          ...
          const x = coluna === 0 ? xColuna1 : xColuna2;

          const y = posicoesY[linha];
          ...
          doc.rect(x, y, larguraImg, alturaImg + alturaDescricao);

          // Linha que separa a foto da descrição
          doc.line(x, y + alturaImg, x + larguraImg, y + alturaImg);
          ...
          const descricaoFoto = foto.desc ? foto.desc : " ";

          const textoLegenda = `Figura ${fCount++} - ${descricaoFoto}`;

          const legendaFormatada = doc.splitTextToSize(textoLegenda, 8.5);

          doc.text(legendaFormatada, x + 0.1, y + alturaImg + 0.45);
        });
  ...
  if (isVistoria && paginaIndiceVistoria) {
    ...
    doc.setPage(paginaIndiceVistoria);
    ...
    const escreverLinhaIndice = (texto, pagina, negrito = false) => {
      ...
      while (
        textoFinal.length > 1 &&
        doc.getTextWidth(textoFinal) > larguraMaxTexto
      ) {
      ...
      // Texto principal
      doc.text(textoFinal, xTexto, yIndice);

      // Calcula onde o texto terminou
      const larguraTexto = doc.getTextWidth(textoFinal);

      const inicioPontilhado = xTexto + larguraTexto + 0.15;

      // Número da página
      const paginaTexto = String(pagina);

      const larguraPagina = doc.getTextWidth(paginaTexto);

      const fimPontilhado = xPagina - larguraPagina - 0.25;

      // =================================================
      // PONTILHADO
      // =================================================

      if (fimPontilhado > inicioPontilhado) {
        doc.setFont("helvetica", "normal");
        ...
        const quantidadePontos = Math.max(
          1,
          Math.floor((fimPontilhado - inicioPontilhado) / larguraPonto),
        );

        doc.text(".".repeat(quantidadePontos), inicioPontilhado, yIndice);
      }
      ...
      doc.text(paginaTexto, xPagina, yIndice, {
        align: "right",
      });
    ...
    escreverLinhaIndice("1 - Objetivo", 2);

    // ========================================================
    // 2.X - ANEXOS
    // ========================================================

    indiceVistoria.forEach((item) => {
      escreverLinhaIndice(
    ...
    escreverLinhaIndice("3.0 - Observação", paginaObservacaoVistoria, false);

    // ========================================================
    // 3.1 - CONCLUSÕES
    // ========================================================

    escreverLinhaIndice("3.1 - Conclusões", paginaConclusoesVistoria, false);
  ...
  if (isVistoria && modo === "final") {
    ...
    const textoObservacao = "XXXXXXXXXXXXX";

    const linhasObservacao = doc.splitTextToSize(textoObservacao, 18);

    doc.text(linhasObservacao, 1.5, 7.2, {
      lineHeightFactor: 1.4,
    });
    ...
    doc.text("3.1 – CONCLUSÕES", 1.5, 9.2);

    doc.setFont("helvetica", "normal");

    doc.setFontSize(11);

    const textoConclusao = "XXXXXXXXX";

    const linhasConclusao = doc.splitTextToSize(textoConclusao, 18);

    doc.text(linhasConclusao, 1.5, 10.2, {
      lineHeightFactor: 1.4,
    });
    ...
    if (assinaturaRealizada && assinaturaCanvas) {
      ...
      doc.addImage(assinaturaImagem, "PNG", 6.5, 15.0, 8, 2.5);
    }
    ...
    doc.text("Albetan Reis", 10.5, 18.7, {
      align: "center",
    });

    // Cargo
    doc.text("Engenharia de Manutenção", 10.5, 19.4, {
      align: "center",
    });
    ...
    const agora = dVal ? new Date(`${dVal}T12:00:00`) : new Date();
    ...
    doc.text(dataPorExtenso, 19.0, 24.5, {
      align: "right",
    });
  ...
  if (modo === "final" && !isVistoria) {
    ...
    doc.text(" ", 10.5, 6, { align: "center" });

    doc.setFont("helvetica", "normal");
    doc.setFontSize(9);

    doc.text("Relatório revisado pelo responsável da loja.", 10.5, 7, {
      align: "center",
    });
    ...
    if (assinaturaRealizada && assinaturaCanvas) {
      ...
      doc.addImage(assinaturaImagem, "PNG", 5.5, 10, 10, 3.3);
    }
    ...
    doc.text("Assinatura do Responsável", 10.5, 14.7, { align: "center" });
    ...
    doc.text(`Data: ${new Date().toLocaleDateString("pt-BR")}`, 10.5, 15.5, {
      align: "center",
    });
  }
